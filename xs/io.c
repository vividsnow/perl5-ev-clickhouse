...
static int pipeline_advance(ev_clickhouse_t *self) {
    ...
    if (!ngx_queue_empty(&self->send_queue)) {
        ...
        if (self->on_query_start && !IS_KEEPALIVE_CB(dispatched_cb)) {
            ...
            FREETMPS; LEAVE;
            /* Handler may have called finish() (destroyed) or reset()
             * (still alive but on a fresh connection — connect_gen
             * bumped). Either way, don't transfer this send's insert
             * payload into the rotated/dead struct. */
            if (check_destroyed(self)) return 1;
            if (self->connect_gen != gen_before) {
                /* The cb was already moved to cb_queue (and freed by
                 * cancel_pending firing the err cb during reset).
                 * Drop the local send entry — its insert payload
                 * mustn't transfer into the rotated connection.
                 * query_id was already moved to self->last_query_id. */
                Safefree(send->data);
                CLEAR_INSERT(send);
                release_send(send);
                return 0;
