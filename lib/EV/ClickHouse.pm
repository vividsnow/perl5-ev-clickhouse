...
sub new {
    ...
    # Pass-through setters: truthy values trigger numeric/boolean setters.
    for my $opt (qw(compress tls tls_skip_verify auto_reconnect
        ...
        my $val = delete $args{$opt} or next;
        my $setter = "_set_$opt";
        $self->$setter($val);
    }
    ...
    if ($host !~ /^[\d.]+$|^\[?[0-9a-fA-F:]+\]?$/
        ...
        $r->resolve($host, sub {
            ...
            if ($status != 0 || !@addrs) {
                $weak2->skip_pending;
                eval { $weak2->on_error->("DNS resolution failed for '$host'") };
                return;
            }
            my ($v4) = grep /^[\d.]+$/, @addrs;
...
sub for_table {
    ...
    die "for_table: invalid table name '$table'"
        unless $table =~ $RE_TABLE;
    my $sql = "describe table $table format TabSeparated";
    $self->query($sql, sub {
        my ($rows, $err) = @_;
...
# Strict variant of query() for DDL/DML: croaks immediately if the
# connection isn't usable, otherwise behaves like query() with the
# (undef, $err) callback shape. Mostly a readability marker for
# migration scripts.
sub ddl {
    my ($self, $sql, $cb) = @_;
    die "Usage: \$ch->ddl(\$sql, \$cb)" unless ref($cb) eq 'CODE';
    $self->query($sql, $cb);
}
...
sub parse_uri {
    ...
    if (defined $qs && length $qs) {
        for my $pair (split /&/, $qs) {
            my ($k, $v) = split /=/, $pair, 2;
            $out{settings}{$k} = defined $v ? _uri_unescape($v) : 1;
        }
    }
    return \%out;
}
...
sub _new {
    ...
    bless {
        ...
        low_water      => $opts{low_water}      || (($opts{high_water} || 0) / 2),
        columns        => $columns,
        high_water_active => 0,
        buffer         => [],
        in_flight      => 0,
...
sub _flush {
    ...
    my $cb = sub {
        ...
        # Fire any await_drain waiters that asked to be notified when
        # the buffer dropped below low_water (or empty, if unset).
...
sub await_drain {
    ...
    # ever fire otherwise.
    return $cb->() if !$self->{in_flight} && @{ $self->{buffer} } <= $low;
...
# on dedupe miss). $cb receives ($rows, $err) as usual.
sub hedged_query {
    my ($self, $sql, @rest) = @_;
    my $cb       = pop @rest;
    my %opts     = @rest;
...
# by member index. Skip-dead and per-query settings are honoured.
sub fan_out {
    my ($self, $sql, @rest) = @_;
    my $cb       = pop @rest;
    my %opts     = @rest;
    ...
    for my $i (0 .. $#c) {
        my $ch = $c[$i];
        my $obs = $self->_cb_observer($ch, sub {
...
=head2 parse_uri
    ...
    my $parsed = EV::ClickHouse->parse_uri(
        'clickhouse+native://u:p@host:9000/db?max_threads=4'
    );
    # $parsed = {
...
validate user-supplied URIs without opening a connection. The
resulting hash can be passed directly to L</new> as constructor
arguments.
...
C<$ch> - any batch already on the wire still completes normally.
...
flushes before the watermark is observed and C<on_high_water> never
...
up to C<$fraction * delay> randomness to spread thundering-herd retries.
...
C<[A-Za-z_][A-Za-z0-9_]*>; anything else croaks. Internal backticks
are doubled per ClickHouse's quoting rule.
...
with whichever returns first. Extra completions are silently
discarded; the error path fires only if I<every> member fails.
