...
# a connection pool with idempotent tokens (so a reconnect-driven retry
# can't double-write). Demonstrates the complete reliable-ingest shape:
#
#   producer ---push_row---> Streamer (batch + high_water)
#                           |
#                           v
#                       Pool::insert (idempotent => 1)
#
...
my $pool = EV::ClickHouse::Pool->new(
    ...
    size     => 4,
    settings => { idempotent => 1 },         # auto-token every insert
);
