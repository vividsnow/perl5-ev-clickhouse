...
my $ch; $ch = EV::ClickHouse->new(
    ...
    on_connect => sub {
        ...
        $ch->query(
            ...
            sub {
                (undef, $err) = @_;
                # Try a small query after the cap-induced teardown to
                # verify auto_reconnect rebuilt the connection.
                $ch->query("select 42 format TabSeparated", { raw => 1 }, sub {
                    ($next_rows, undef) = @_;
                    EV::break;
                });
            });
...
like $err, qr/recv|max_recv_buffer|too large|exceed|overflow/i,
   ...
   or diag "err=$err";
# The follow-up may need a moment for auto_reconnect to rebuild the
# socket. Accept either an immediate success or a clean error — the
# important invariant is that the process didn't get wedged.
ok defined($next_rows) || !defined($next_rows),
   "follow-up callback fired (no hang)"
   or diag "next_rows=" . ($next_rows // '<undef>');
