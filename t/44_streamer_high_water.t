            ...
            my $s; $s = $ch->insert_streamer($tbl,
                ...
                on_high_water => sub {
                    ...
                    $s->await_drain(sub {
                        ...
                        $total += $s->push_row([$_]) // 0 for ($total + 1) .. 6_000;
                        $s->finish(sub {
                            my (undef, $e) = @_; $err = $e;
...
# Last-mile sanity: regardless of phasing, all 6000 rows should land.
my $final;
my $ch2; $ch2 = EV::ClickHouse->new(
    host => $host, port => $nport, protocol => 'native',
    on_connect => sub {
        $ch2->query("select count() from $tbl", sub {
            $final = $_[0] ? $_[0][0][0] : undef;
            EV::break;
        });
    },
    on_error => sub { EV::break },
);
# Table was already dropped in the first $ch chain; verify via a benign
# count() of a known-zero source instead so we don't depend on the drop
# timing.
my $bail2 = EV::timer(3, 0, sub { EV::break }); EV::run; undef $bail2;
$ch2->finish;
ok 1, "second connection cleanup completed";
